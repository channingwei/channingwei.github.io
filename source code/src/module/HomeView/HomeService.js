...
ckApp.factory('HomeService', ['rt', function (rt) {
   ...
   // var fluid = function() {
      ...
      var run = function () {
         ...
         var ii = num_particles;
         while(ii--)
            particles[ii].first_process();
         ii = num_particles;
         while(ii--)
            particles[ii].second_process();
      ...
      Particle.prototype.second_process = function () {
         var force = 0, force_b = 0, cell_x = Math.round(this.x / spacing), cell_y = Math.round(this.y / spacing), close = [];
         ...
         force = (force - 3) * 0.5;
         for (var i = 0, ll = close.length; i < ll; i++) {
      ...
      function _init(canvasId, w, h) {
         ...
         for (var ii = 0; i < num_x * num_y; ii++) {
            grid[ii] = {
               length: 0,
               close: []
            };
         }
         for (var iii = 0; i < GROUPS.length; iii++ ) {
            for (var k = 0; k < GROUPS[iii]; k++ ) {
               particles.push(
                  new Particle(
                     iii,
